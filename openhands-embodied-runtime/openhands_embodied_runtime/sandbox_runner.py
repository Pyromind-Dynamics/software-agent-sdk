...
from openhands_embodied_runtime.planning import (
    ALIGNMENT_WARNING_GAP_S,
    DEFAULT_ALIGNMENT_MAX_GAP_S,
...
def run_full(
    ...
    *,
    ...
    _write_phase_report(run_dir, phase, report)
    return report
...
def _alignment_policy() -> dict[str, float]:
    return {
        "warning_gap_s": ALIGNMENT_WARNING_GAP_S,
        "reject_gap_s": DEFAULT_ALIGNMENT_MAX_GAP_S,
    }


def _publish_validated_dataset(
    source: Path,
    target: Path,
    *,
    run_id: str,
    ...
    if target.exists():
        ...
        if validation.valid and _same_dataset(source, target):
            return
        if not validation.valid:
            raise ValueError(f"existing target dataset is invalid: {target}")
