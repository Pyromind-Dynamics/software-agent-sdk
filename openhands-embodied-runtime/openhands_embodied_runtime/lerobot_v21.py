...
def materialize_finalized_lerobot_v21_episode(
    ...
    *,
    ...
    if prepared.source_type == SourceType.SELF_COLLECTED:
    ...
    elif prepared.source_type == SourceType.LEROBOT_V21:
        adapter = LeRobotV21Adapter(source)
        adapter.select(prepared.episode_id)
        episode_dir = adapter.source
        source_states = load_lerobot_state_vectors(source, prepared.episode_id)
        states = [
            np.asarray(
                source_states[mapping.source_frame_index],
                dtype=np.float32,
            )
            for mapping in prepared.timeline_mapping
        ]
...
def _selected_video_indexes(
    ...
    *,
    ...
    if plan.source_type == SourceType.LEROBOT_V21:
        ...
        for name, stream in plan.streams.items():
            ...
            if stream.status != StreamStatus.AVAILABLE:
                continue
            if stream.frame_count != plan.source_timeline.frame_count:
                raise ValueError(
                    f"{name} MP4 frame count does not match the source timeline"
                )
            selected[name] = selected_primary
        return selected
...
def _vector_feature(plan: EpisodePlan, field: str) -> dict[str, Any]:
    ...
    return {
        ...
        "units": [feature.unit for feature in feature_list],
    }
