...
通用入口用合成图与正常参考的绝对差分检查实际变化。两材料示例另保留灰度
高通残差及有符号数据作为观察图；正常线路边缘也会响应，不能作为缺陷真值。
...
- 源模板、候选几何和实际成图分别验证，保存测量值与失败/不确定原因；结构或
  外观语义由策略定义，不能只检查理想掩码。几何通过不代表业务类别正确。
...
模板合成的 generation/review/quality 分目录保存；业务报告用 `synthesis_*`
命名，避免覆盖执行器报告。独立交付包包含计划、候选、资产、血缘、阶段失败
证据及最终 `reviewed.quality.json`，具体契约见[模板合成](template-synthesis.md)。
图片记录明确正常参考图、异常图、缺陷掩码和差分
