...
1. **创建**：`sandbox_create(name=..., wait_timeout=600)`；`image` 不传时优先使用
   服务端配置的 `PYROMIND_SANDBOX_DEFAULT_IMAGE`，未配置时按集群选择默认镜像：
...
3. **执行命令**：`sandbox_terminal(sandbox_id=..., command="pip list",
   ...
   `timeout_seconds`（默认 60；长任务可显式设置更大的值）
...
- `sandbox_terminal` 的 `timed_out=True` 表示命令超时（输出可能不完整）；
  普通命令失败看 `returncode`
- 目录列表用 `sandbox_terminal`；写入和删除分别使用
  `sandbox_write_file`、`sandbox_delete_file`
- 长任务不要依赖一次超长阻塞调用；用 `nohup` 启动、保存 PID，然后通过
  `sandbox_terminal` 查询进程并用 `sandbox_read_file` 读取日志
