...
3. **生成配置**：按样本结构生成 `label_config.xml`；需要改绑定时同一层再写
   ...
   看不到预标注、导出时字段为空。
   `<Choice>`/`<Label>` 的值域只是**模板**，不必穷举、也不必和数据严格一致：数据里
   出现、列表里没有的值，create 会自动补进**项目上**的配置（控件名和布局不动），
   并在返回里报 `widened=<控件>:<值>`，预标注照常渲染。你写的 XML 文件**不会被
   改动**。所以按数据实际携带的类别给一个合理的起步值域即可（`preview_dataset`
   看到的判定值/类别、用户给出的缺陷类型、上游标签分布结论），不必为了对齐模板去
   洗数据、改 XML、反复重试；也不要凭常识刻意收窄。
