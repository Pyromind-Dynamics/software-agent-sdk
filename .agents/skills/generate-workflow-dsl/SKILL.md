...
- Skill 资源只通过 `skills_read(skill_name="generate-workflow-dsl", path="references/...")`
  读取；`references/` 列表是索引，不是必读清单，只读当前决策需要的文件。
- 查询知识库使用 `grep` 和 `file_editor` 及逻辑路径 `knowledge/...`；不要用它们读取
  `.agents/skills/...`。
- 禁止使用 `terminal`、`find`、`ls`、`cat` 或 `apply_patch` 扫描、读取或修改知识库和技能目录。
- 禁止请求、拼接或传递知识库和技能目录的宿主机绝对路径。
- 仅在下文兜底条件触发时，用 `file_editor(command="view")` 打开已确定的节点文档逻辑路径；
  不要先扫描整个目录。

按需选择 reference，不要为了确认而全部读取：

| reference | 只在以下情况读取 |
|-----------|------------------|
| `node-reference.md` | 需要节点端口、参数或常用拓扑；常规生成优先只读这一份节点资料 |
| `parameter-decision.md` | 需要根据数据规模、序列长度或 GPU 预算自动选择/修改训练参数 |
| `platform-contract-overrides.md` | `node-reference.md` 缺少相关契约，或校验结果与现有资料冲突 |
| `example-workflows.md` | `node-reference.md` 没有匹配拓扑，或需要 DPO、GRPO 等非常规完整示例 |
...
bench 工作流结构如下；只有需要非常规完整模板时才读
[references/example-workflows.md](references/example-workflows.md) 示例6：
...
- 优先使用 [references/node-reference.md](references/node-reference.md) 的常用拓扑；仅当其中
  没有匹配拓扑时，才读取 [references/example-workflows.md](references/example-workflows.md)。
  参数名、输入端口和输出端口以节点契约为准。
...
需要根据数据或资源自动配参时，才读
[references/parameter-decision.md](references/parameter-decision.md)，按表填 lora_rank、
batch_size、learning_rate、epoch 等参数；校验现有工作流或保留已有有效参数时不要读。速记原则：
...
检索优先级：**运行时校验结果 → [节点速查](references/node-reference.md) → 按需读取
[平台契约覆盖层](references/platform-contract-overrides.md) → 定向读取单个节点文档 →
必要时读取示例或宽泛 grep**。先依据节点速查生成并校验，不要预读所有参考资料或节点文档。

- 节点速查表和常用参数见 [references/node-reference.md](references/node-reference.md)
- 节点契约文档路径固定：`knowledge/nodes/<NodeType>/<NodeType>.md`。若节点速查已覆盖所需
  参数和端口，不要再打开独立节点文档。只有节点未收录、所需字段/类型/范围缺失、校验错误
  明确指向该节点/字段，或平台契约冲突时才定向读取；首次校验前不要逐个确认节点
