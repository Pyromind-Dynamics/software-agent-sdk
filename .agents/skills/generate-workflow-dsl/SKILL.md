...
2. **数据集标识**：不调用 `preview_dataset`。下面是已知可直接使用的数据，不限制用户使用
   ...
   | 节点 | 参数 | 已知数据集 |
   |------|------|------------|
   | CloneAndCacheDataset | `dataset` | `pyromind/alpaca-gpt4-llm-demo`、`pyromind/geometry-vqa-vlm-demo`、`pyromind/self-cognition` |
   | DownloadAndCacheDataset | `dataset_name` | `pyromind/easyhard-24k`、`pyromind/agentic-tool-call-dataset-12k` |

   三个 Clone 数据集是 Pyromind 平台自带测试集：`geometry-vqa-vlm-demo` 用于多模态，另外
   两个用于 LLM 场景。
