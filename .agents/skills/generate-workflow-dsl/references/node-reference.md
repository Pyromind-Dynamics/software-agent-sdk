...
推理工作流时需要高频读取的契约。表中 `必填`、`可选` 和输出端口均直接来自该快照；具体
类型、默认值、范围及运行时约束仍以 `knowledge/nodes/<NodeType>/<NodeType>.md` 和
[platform-contract-overrides.md](platform-contract-overrides.md) 为准，发生冲突时按 SKILL.md
规定的检索优先级处理。
...
当前 `node.json` 中下列节点的 `gpu_product` 枚举一致：

- `ModelTrainSFTNode`、`ModelTrainDPONode`、`ModelTrainGRPONode`、`ModelTrainOPDNode`
- `ModelMergeLoraNode`、`VLLMInference`、`Inference`、`AccelerateGpuNode`

允许值为 `NVIDIA-H100-NVL`、`NVIDIA-L40S`，默认 `NVIDIA-H100-NVL`。训练和推理节点的
`gpu_count` 范围是 1～8；`ModelMergeLoraNode.gpu_count` 固定为 1。
