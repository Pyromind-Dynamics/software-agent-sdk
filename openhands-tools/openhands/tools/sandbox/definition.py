...
class SandboxObservation(Observation):
    ...
    type: str | None = Field(default=None, description="Sandbox type ('custom').")
...
# ---------------------------------------------------------------------------
# Sandbox lifecycle, file operations, and terminal execution
# ---------------------------------------------------------------------------
...
class SandboxCreateAction(Action):
    ...
    image: str | None = Field(
        default=None,
        description=(
            "Container image to run. Defaults to the deployment-configured image, "
            "then to the cluster Jupyter-lab image when deployment has no override."
        ),
    )
...
class SandboxTerminalExecutor(
    ...
    def __call__(
        ...
        try:
            client = self._sandbox_client(conversation)
            command = (
                f"cd {shlex.quote(action.cwd)} && {action.command}"
                if action.cwd
                else action.command
            )
        ...
        return [
            cls(
                description=(
                    ...
                    "deployment or cluster default image is used. The sandbox "
                    "can run commands (sandbox_terminal / agent terminal) and "
